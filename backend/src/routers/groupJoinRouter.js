...
groupJoinRouter.put('/mygroups/:groupId/:userId/approval', loginRequired, async (req, res,next) => {
  try {
    const groupId = req.params.groupId;
...
groupJoinRouter.delete('/mygroups/:groupId/:userId/rejection', loginRequired, async (req, res, next) => {
  try {
    const groupId = req.params.groupId;
    const userId = req.params.userId;
